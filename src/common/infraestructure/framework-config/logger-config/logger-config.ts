...
export class FileLogger extends ConsoleLogger {
        ...
        process.on('unhandledRejection', (reason: any) => {
            ...
            if (reason instanceof AggregateError) {
                ...
                const errors = reason.errors.map((err: any) => {
                    return err.stack as string || err.message as string || JSON.stringify(err);
                });

                this.error(`Unhandled Promise Rejection: ${errors.join(', ')}`);
                this.writeToFile('ERROR', `Unhandled Promise Rejection: ${errors.join(', ')}`);
            } else {
                const errorMessage = reason instanceof Error ? reason.stack || reason.message : JSON.stringify(reason);
                this.error(`Unhandled Promise Rejection: ${errorMessage}`);
                this.writeToFile('ERROR', `Unhandled Promise Rejection - ${errorMessage}`);
            }
        });
    ...
    private writeToFile(level: string, message: string, context?: string) {
        ...
        const color = level == LogLevel.LOG ? '\x1b[32m' : level == LogLevel.ERROR ? '\x1b[31m' : '\x1b[33m';
