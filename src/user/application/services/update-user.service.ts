...
export class UpdateUserService extends IService<UpdateUserRequestDto, void> {
	...
	public async execute(value: UpdateUserRequestDto): Promise<Result<void>> {
		const userModel = await this.ormUserQueryRepository.findUserById(
			value.id,
		)
		...
		if (value.email) {
			const userExist =
				await this.ormUserQueryRepository.existUserByEmail(value.email)
			if (userExist.isError || userExist.Value)
				return Result.fail(
					new UserEmailAlreadyExistsException(value.email),
				)
		...
		if (value.role)
			user.updateRole(UserRoleVo.create(value.role.toString()))

		const save = await this.ormUserCommandRepository.saveUser({
			user,
